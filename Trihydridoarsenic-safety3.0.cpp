...
DWORD WINAPI coolInvert0(LPVOID lpParam) {
	HDC hdc = GetDC(0);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	if (MessageBoxW(NULL, L"Run Trihydridoarsenic-safety3.0.exe GDI only.", L"Trihydridoarsenic-safety3.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure?", L"Trihydridoarsenic-safety3.0.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
				...
				Sleep(100);
				HANDLE thread11 = CreateThread(0, 0, patblt, 0, 0, 0);
				sound8();
				Sleep(30000);
				TerminateThread(thread11, 0);
				CloseHandle(thread11);
				Sleep(100);
				HANDLE thread12 = CreateThread(0, 0, coolInvert0, 0, 0, 0);
				sound9();
				Sleep(30000);
